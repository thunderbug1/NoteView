...
const App = {
    ...
    async init() {
        ...
        try {
            ...
            if (initialized) {
                ...
                const recovered = await this.offerVaultRecovery();
                if (recovered) {
                    await this.completeInitialization();
                } else {
                    this.showManageVaultsModal();
    ...
    async initializeInBackground() {
        ...
        try {
            ...
            if (initialized) {
                ...
                if (recovered) {
                    this.updateCaptureWithVaultName();
                    if (window.SyncManager) window.SyncManager._setStatus('syncing', 'Initializing sync...');
                    await this.completeInitialization(true);
                    this._updateCaptureSyncStatus();
                } else {
                    this.showManageVaultsModal();
    ...
    _syncIcons: {
        ...
        syncing: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 4 23 10 17 10"></polyline><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"></path></svg>',
    ...
    async offerVaultRecovery() {
        let backup;
        try {
            backup = await Store.checkVaultRecovery();
        } catch (e) {
