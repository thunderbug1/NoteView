...
- **`SyncManager`** (`js/syncManager.js`) — Orchestrates automatic git remote syncing. Handles triggers (startup pull, interval, idle, commit threshold), network status tracking, toolbar indicator, and three-way merge conflict resolution with per-file UI. Dirty-tree pull conflicts recover via a safe forced-pull path that stashes unique local edits to `refs/noteview/recovery/*` before resetting to remote (see docs/git-integration.md). Delegates push/pull to `GitRemote`.
